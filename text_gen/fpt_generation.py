...
from config.constants import EMOTIONAL_SUPPORT, RECOMMENDATION, NEGOTIATION, FPT


class FPTConfigForGeneration(GenerationConfig):
    prompt = "This is the prompt and subjected to be changed"
    temperature = 0.0


class FPTGeneration(LLMGeneration):
    """Response generation backed by FPT cloud (Llama-3.3-70B-Instruct)."""
    ...
    def generate_response(self, instance):
        ...
        response = call_llm(messages, n=1,
                            ...
                            model_type=FPT)
        logger.debug(f"FPT Response Generation Time: {time.time() - t:.2f}s")
        return response[0]
