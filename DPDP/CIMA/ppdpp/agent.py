...
from ICDM2026.DPDP.CIMA.ppdpp.utils import *
from ICDM2026.DPDP.CIMA.ppdpp.prompt import ESConvAct, CIMAAct, CBAct
import ICDM2026.DPDP.CIMA.ppdpp.utils as utils
from ICDM2026.DPDP.CIMA.ppdpp.env import system_role, user_role
from ICDM2026.DPDP.CIMA.ppdpp.qnet import Qnet

from ICDM2026.DPDP.CIMA.mcts.gdpzero import GDPZero
from ICDM2026.DPDP.CIMA.mcts.core.mcts import OpenLoopMCTS
from ICDM2026.DPDP.CIMA.mcts.utils.utils import dotdict
...
class PPDPP(nn.Module):
    ...
    def forward(self, state_ids, attention_mask, next_state_ids=None, next_attention_mask=None, actions=None, rewards=None, dones=None):
        ...
        if actions is not None:
            ...
            next_outputs = self.policy(input_ids=next_state_ids, attention_mask=next_attention_mask)
            next_pooled_output = next_outputs[1]
            next_pooled_output = self.dropout(next_pooled_output)
            # max_actions = self.Q_head(next_pooled_output).max(1)[1].view(-1, 1) 
            # max_next_q_values = self.Q_head_target(next_pooled_output).gather(1, max_actions).view(-1).detach()
            max_next_q_values = self.Q_head(next_pooled_output).max(1)[0].view(-1).detach() 
            
            td_target = rewards + self.gamma * max_next_q_values * (1 - dones)
            loss_fct = MSELoss(reduction='mean')
            critic_loss = loss_fct(qa_value.view(-1), td_target.view(-1))
            ...
            td_delta = td_target.view(-1) - qa_value.view(-1)
