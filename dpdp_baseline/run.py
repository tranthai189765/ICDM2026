"""DPDP baseline runner for HMOD bargain / recommendation scenarios.

Mirrors PPDPP/run.py but routes per-turn action selection through
`dpdp_baseline.mcts.DPDPPlanner`, which switches between the PPDPP policy
network (System 1) and a 1-ply MCTS lookahead (System 2) using a top-1/top-2
probability margin gate. Same CLI surface as PPDPP/run.py so the
`scripts/standardize_benchmark.py` reader treats the outputs identically.

Example (HMOD bargain test, evaluation only):
    python -m dpdp_baseline.run \\
        ...
        --num_cases 40 --objective uniform \\
        --judge_model rule --planner_mode dual \\
        --output_dir outputs/dpdp_eval_bargain --do_eval
"""
...
# Bootstrap PPDPP + repo root so we can reuse the existing env / utils / etc.
...
from dpdp_baseline.agent import DPDPAgent  # noqa: E402
from dpdp_baseline.mcts import DPDPPlanner  # noqa: E402
...
def _select_action(policy: DPDPAgent, planner: DPDPPlanner, env, args, is_test: bool):
    """Single-call policy+planner dispatch for both train and eval loops."""
    mode = getattr(args, "planner_mode", "dual")
    if mode == "policy":
        return policy.select_action(env.conversation, is_test=is_test), "policy"
    if mode == "mcts":
        action_labels, probs = policy.compute_probs(env.conversation)
        result = planner.select_action(
            env=env,
            action_labels=action_labels,
            probs=probs,
            eta=float(args.mcts_eta),
            force_mcts=True,
        )
        return result["action"], result["planner"]
    # dual: control gate decides
    action_labels, probs = policy.compute_probs(env.conversation)
    result = planner.select_action(
        env=env,
        action_labels=action_labels,
...
def evaluate(args, dataset, policy, planner, filename, i_episode, train_env,
    ...
    if "vicuna" in [args.system, args.user, args.critic] or "llama2" in [args.system, args.user, args.critic]:
        test_env = Env(
            args, dataset, mode="test",
            env_model=train_env.vicuna_model,
            env_tokenizer=train_env.vicuna_tokenizer,
        )
    else:
    ...
    if test_size <= 0:
        print("[dpdp] empty test split; skipping eval")
        return [0.0, 0.0, 0.0]
    ...
    with open(rec_path, "w") as rec_file:
        for test_num in tqdm(range(test_size)):
            ...
            for t in count():
                action, planner_label = _select_action(
                    policy, planner, test_env, args, is_test=True,
                )
    ...
    planner_usage = dict(planner.usage)
    if episode_records:
        metrics_payload = {
            "model": "DPDP",
            "epoch": i_episode,
        ...
        print(f"DPDP structured outputs written to {run_dir}")

    print("planner usage:", planner_usage)
...
def train(args, config, dataset, filename, tokenizer):
    env = Env(args, dataset, mode="train")
    set_random_seed(args.seed)
    policy = DPDPAgent(args, config, tokenizer)
    planner = DPDPPlanner(top_k=args.mcts_top_k, cache_size=args.mcts_cache)
    ...
    if args.do_eval:
        ...
        SR15_mean = evaluate(
            args, dataset, policy, planner, filename, 0, env,
    ...
    for train_step in range(start_step, args.max_steps + 1):
        ...
        for i_episode in tqdm(range(args.sample_times), desc="sampling"):
            ...
            epi_reward = 0
            for t in count():
                action, _planner_label = _select_action(
                    policy, planner, env, args, is_test=False,
                )
        ...
        print("planner usage (train):", planner.usage)
        ...
        if train_step % args.eval_num == 0:
            ...
            SR_all = evaluate(
                args, dataset, policy, planner, filename, train_step, env,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="DPDP baseline runner")
    # General
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/dpdp_eval")
    ...
    # DPDP planner
    parser.add_argument("--planner_mode", type=str, default="dual",
                        choices=["policy", "mcts", "dual"],
                        help="policy = System 1 only, mcts = always System 2, dual = control gate.")
    parser.add_argument("--mcts_eta", type=float, default=0.3,
                        help="Confidence margin threshold (top1 - top2) for invoking MCTS.")
    parser.add_argument("--mcts_top_k", type=int, default=2,
                        help="Top-K policy candidates expanded during MCTS lookahead.")
    parser.add_argument("--mcts_cache", type=int, default=256,
                        help="Bounded LRU size for action simulation cache.")
    # PPDPP infra knobs
...
def main():
    ...
    print("planner_mode:", args.planner_mode, "eta:", args.mcts_eta,
